...
#'
#' @export
closed_ts_plot_lsoa_binary <- function(df        = ed_attendances_by_mode_site_measure,
    ...
    results <- list()
    ## Check to see if LSOA level data has been passed to the function and if
    ## so now collapse the data
    ...
    ## If we have LSOA level data we now summarise and plot
        ...
        ## Reuse code written in closed_models() and closed_stata_negbin(), if
        ...
        results$plot <- results$plot +
                        ...
                        geom_smooth(data = dplyr::filter(df, missing != 1),
                                    aes(x = relative.month,
                                        y = value,
                                        linetype = binary.diff), method = 'loess', color = 'black') +
            ## geom_smooth(aes(linetype = binary.diff), method = 'loess', color = 'black') +
    ...
    if(legend == FALSE){
        ...
        results$plot <- results$plot +
                        guides(colour = guide_legend('Difference in time to ED'),
                               shape = guide_legend('Difference in time to ED'),
                               linetype = guide_legend('Difference in time to ED'))
    }
    ## Return results
