...
* Filter data -/+ 2 years of the minimum and maximum closure step (the same when restricted to one site), might already have this working but need to check returned data frame (all.attendance.any.matched).
...
* Cross-checke results from panelAR function against those of the prais function.
...
* Rotate text on Time to ED histograms.
* How to lay out time-series plots for the pooled analyses, certainly for the unmatched panel these by the group around the closed ED.
* Once spurious data points resolved need to work out how to plot discontinuous lines (will involve adding a group variable to each cluster, so remember to derive that when excluding data points).
...
* Derive a sensible and clean tabulated layout for the results.