...
Data was provided by [HSCIC](https://www.gov.uk/government/organisations/health-and-social-care-information-centre) (now [NHS Digital](https://digital.nhs.uk/)), Ambulance Services (**Which?**) and [ONS](https://www.ons.gov.uk/) and were combined and summarised into either counts of events per month, mean of events per month or the proportion of particular outcomes for observed events for the different indicators.
...
|----------------------------------|-----------|----------------------------------|------------------------|--------------|--------------------------------
| **Ambulance Service Performance** | AS       | `ambulance mean times`           | `call_to_dest`           | `mean`     | Mean time from 999 call to destination (ED).
|                                   |          | `ambulance mean times`           | `call_to_scene_any`      | `mean`     | Mean time from 999 call to scene for all calls.
|                                   |          | `ambulance mean times`           | `call_to_scene_conveying`| `mean`     | Mean time from 999 call to scene for calls that resulted in conveynace.
|                                   |          | `ambulance mean times`           | `scene_to_dest`          | `mean`     | Mean time from scene to destination (ED).
|                                   |          | `ambulance mean times`           | `dest_to_clear`          | `mean`     | Mean time from destination (ED) to being clear.
...
|                                   |          | `length of stay`                 | `mean`                 | `mean`       | Mean length of stay of admissions (**all emergency or ed attendances?**)
|                                   |          | `length of stay`                 | `median`               | `median`     | Mean length of stay of admissions (**all emergency or ed attendances?**)
| **Risk of Death**                 | ONS/HES-ONS | `case fatality ratio`            | `any`                  | `proportion` | Proportion of cases for avoidable deaths (see below) that resulted in death. (**Are these of admissions or ED attendances or both?**)
|                                   |             | `case fatality ratio`            | `myocardial infarction`| `proportion` | Proportion of MIs that resulted in death. (**Are these of admissions or ED attendances or both?**)
|                                   |             | `case fatality ratio`            | `stokre cva`           | `proportion` | Proportion of Stroke/CVA's that resulted in death. (**Are these of admissions or ED attendances or both?**)
|                                   |             | `case fatality ratio`            | `any trauma sec`       | `proportion` | Proportion of trauma (RTA, Falls in those < 75, Non-superficial head injuries **Have I got these right?**) that resulted in death. (**Are these of admissions or ED attendances or both?**)
...
| **??** | **??**       | `ambulance green calls`          | `green calls`            | `count`    | Number of green calls (**Should this be included?**)
| **??** | **??**       | `ambulance green calls`          | `green calls`            | `count`    | Number of green calls (**Should this be included?**)
| **??** | **??**       | `ambulance green calls`          | `not conveyed green calls`| `count`    | Number of green calls that weren't conveyed (**Should this be included?**)
| **??** | **??**       | `ambulance green calls`          | `fraction not conveyed`  | `proportion` | Proportion of green calls that weren't conveyed (**Should this be included?**)
| **??** | **??**       | `ambulance red calls`            | `hospital transfers`     | `count`    | Number of red calls that were for hospital transfers.
| **??** | **??**       | `ambulance red calls`            | `total`                  | `count`    | Number of red calls.
| **??** | **??**       | `hospital transfers`             | `ambulance`              | `count`    |
| **??** | **??**       | `hospital transfers`             | `stays with transfer`    | `count`    |
| **??** | **??**       | `hospital transfers`             | `fraction with transfer` | `proportion` |
| **Risk of Death**                 | ONS/HES-ONS | `sec deaths all 7days`           | `any sec`                | `count`    | All deaths at 7 days for conditions listed below seen in hospital, regardless of where death occurred.
|                                   |             | `sec deaths in cips 7days`       | `any sec`                | `count`    | All deaths in hospital at 7 days for conditions listed below (**Is this correct?**)
|                                   |             | `sec deaths not in cips 7days	`  | `any sec`                | `count`    | All deaths not in hospital at 7 days for conditions listed below who had contact with ED/hospital. (**Is this correct?**)
